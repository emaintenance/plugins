...
howto()
{
   echo "This plugin check if free memory swap is less than warning and critical"
...
MemTotal=$(cat /proc/meminfo | grep SwapTotal | tr -d -c 0-9)
MemFree=$(cat /proc/meminfo | grep SwapFree | tr -d -c 0-9)
...
if [ "$(echo $prctfree | grep "^[[:digit:]]*$")" ]; then
        ...
        if [ "$prctfree" -le "$CRITICAL" ]; then
                echo "Free memory swap : $prctfree %  (< $CRITICAL) $perfdata"
                  exit 2
        elif [ "$prctfree" -le "$WARNING" ]; then
                echo "Free memory swap : $prctfree % (< $WARNING) $perfdata"
                  exit 1
        elif [ "$prctfree" -gt "$WARNING" ]; then
                echo " Free memory swap : $prctfree % $perfdata "
                  exit 0
        else
                howto
                exit 3
